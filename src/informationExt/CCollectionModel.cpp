...
// -------------------------------------------------------------------------------
{
   ...
   //Q_ASSERT( NULLPTR != mpCollection );  // Don't assert here: with win-qt4.6.3 this will lead to an
                                           // assertion always!
   if ( !mpCollection )
      return QModelIndex();
...
// -------------------------------------------------------------------------------
{
   ...
   {
      QString sDropData( QString::fromUtf8( pData->data( TUXCARDS_DRAGDROP_TYPE ) ) );
