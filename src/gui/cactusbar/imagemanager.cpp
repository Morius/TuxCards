...
QString ImageManager::IMAGE_DIR=":/";                // This ':/' is a link to internal used resources.
...
// -------------------------------------------------------------------------------
{
   debug( "[ImageManager] init flowers" );

   QDir dir( IMAGE_DIR );
   if ( !dir.exists() )
   {
      debugWarn( "[ImageManager] No such directory for cactus-flower-files: " + IMAGE_DIR + "."
                 + " Some Features for the cactus will be disabled." );
      return;
   }

   mFlowerNames.clear();

   //Gesamtes Verzeichnis nach Blüten durchsuchen
   ...
   if( !files.isEmpty() )
   {
      ...
      for ( int i = 0; i < files.count(); i++ )
      {
         ...
         if( f.isFile() && (f.suffix()=="gif") && (f.baseName().right(2)=="_0") )
         {
            ...
            debug( "[ImageManager]  .. " + name );

            mFlowerNames.append( name );
         }
...
// -------------------------------------------------------------------------------
{
  ...
  // um die Bilder für die Blüten kümmern; alle Blüten einlesen
   QString flowerName = mFlowerNames.at( CactusTools::ran(0,mFlowerNames.count()-1) );
   for (int i=0; i<5; i++)
   {
