...
// -------------------------------------------------------------------------------
{
   ...
   debugErr( "[CEditor::keyPressEvent] -> " + QString::number(pKeyEv->key()) );
...
//
   ...
   // If
   //   CTRL + 1   or  CTRL + 2   or   CTRL + 3   or   CTRL + 4   or   CTRL + 5
   //   -> applying pre-defined fonts.
   if( ( (pKeyEv->modifiers() == Qt::ControlModifier)  &&  (pKeyEv->key() == Qt::Key_1) ) )
   {
      debugWarn( "[CEditor::keyPressEvent3] CTRL+1 pressed" );
      QFont f;
      f.fromString( CTuxCardsConfiguration::getInstance().getStringValue( CTuxCardsConfiguration::S_PREDEFINED_FONT_1 ) );
      setCurrentFont( f );
   }
   else
   {
