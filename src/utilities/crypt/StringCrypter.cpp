...
{
   debug( "[StringCrypter::encryptString] sInputString='" + sInputString
          + "', sPassWd='" + sPassWd + "', byteArray.size()=" + QString::number(byteArray.size()) );
   ...
   debug( "[StringCrypter::encryptString] byteArray='" + byteArray + "'" );
   ...
   debug( "[StringCrypter::encryptString] sLeftPart='" + sLeftPart + "'" );

   memset( buf_in, 0, BUFFER_SIZE_IN );
   debug( "[StringCrypter::encryptString] buf_in='" + QString::fromLatin1((const char*)buf_in) + "'" );

   memcpy( buf_in, sLeftPart.toLatin1().data(), sLeftPart.length() );
   debug( "[StringCrypter::encryptString] buf_in='" + QString::fromLatin1((const char*)buf_in) + "'" );


   debugWarn( "[StringCrypter::encryptString] --- while ---" );
   while( sLeftPart.length() > 0 )
   {
      sTmp = sTmp.mid( BUFFER_SIZE_IN );
      debugWarn( "[StringCrypter::encryptString] sTmp='" + sTmp + "'" );

      memset( buf_out, 0, BUFFER_SIZE_IN );
      debug( "[StringCrypter::encryptString] buf_out='" + QString::fromLatin1((const char*)buf_out) + "'" );
      ...
      for ( int i = 0; i < iEncodedSize; i++ )
      {
         ...
         debug( "[StringCrypter::encryptString] i='" + QString::number(i) + "', byteArray='" + byteArray + "'" );
      }
